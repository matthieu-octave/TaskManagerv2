...
- **Relations Eloquent :** Utilisation propre des relations `One-to-Many` (Utilisateur -> Projets, Projet -> Tâches, **Catégorie -> Projets**) et `BelongsTo`.
...
- **Rôle Administrateur & Gates :** Un système de rôle simple (`is_admin`) est protégé par une `Gate` et des `Policies` pour donner accès aux sections "Admin" (consultation des messages, CRUD des Clients et **CRUD des Catégories**).
...
- **Gestion des Catégories (CRUD Admin) :** Interface sécurisée pour la gestion complète des catégories de projet.
...
- **Recherche & Filtres :** Le tableau de bord principal permet de filtrer les projets par statut (Actif/Archivé), par étiquette **et par catégorie**, et inclut une barre de recherche.
...
4.  **Préparer la base de données**
    - Lancez les migrations pour créer toutes les tables et peuplez la base avec des données de test (un utilisateur admin, des clients, des tags, **des catégories**...).
        ```bash
